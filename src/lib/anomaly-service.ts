...
export class AnomalyService {
    static async runDetection() {
        ...
        for (const business of businesses) {
            totalAnomalies += await this.runDetectionForBusiness(business.id);
        }

        return totalAnomalies;
    }

    static async runDetectionForBusiness(businessId: string) {
        ...
        if (!business || business.transactions.length < 5) return 0; // Skip if not enough data
        ...
        if (anomalousIndices.length > 0) {
            ...
            if (aiInsights.length > 0) {
                anomaliesFound += aiInsights.length;
                    ...
                    if (!existingAlert) {
                        await prisma.alert.create({
                            data: {
                                ...
                                transactionDetails: simplifiedAnomalies,
                                date: new Date()
                            }
                        });
                    }
